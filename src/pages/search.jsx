    ...
    <div>
      {props?.hits?.length > 0
        ? props.hits.map((hit, index) => {
            ...
            // console.log(hit)
            let higlightedAttributes = Object.keys(hit._highlightResult).filter(
              v =>
                hit._highlightResult[v].matchLevel !== "none" &&
                v !== "title" &&
                v !== "subtitle"
            )
            console.log(higlightedAttributes)
...
const RefinementLisst = ({ items, refine, createURL, currentRefinement }) => {
  ...
  return (
    <nav className="breadcrumbs" style={{ marginBottom: "var(--space8)" }}>
      <ul className="bread-crumbs !whitespace-normal flex-wrap text-primary justify-content-between gap-1">
